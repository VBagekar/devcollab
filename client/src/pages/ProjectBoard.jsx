...
function DroppableColumn({ id, children }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`flex flex-col gap-2 min-h-32 rounded-lg transition-colors ${isOver ? 'bg-blue-50' : ''}`}
    >
      {children}
    </div>
  )
...
export default function ProjectBoard() {
  ...
  const handleDragEnd = async (event) => {
    ...
    try {
      await api.put(`/api/projects/${id}/tasks/${draggedTask._id}`, { status: newStatus })
    } catch {
  ...
  const handleCreateTask = async (status) => {
    ...
    try {
      setCreating(true)
      const res = await api.post(`/api/projects/${id}/tasks`, { title: newTaskTitle, status })
        ...
        <div className="border-b border-gray-200 bg-white px-6 py-4">
          <div className="flex items-center justify-between max-w-7xl mx-auto">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{tasks.length} tasks</span>
            </div>
            <button
              onClick={() => { setNewTaskColumn('todo'); setShowNewTask(true) }}
                  ...
                  <div key={col.id} className="flex flex-col">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded-full font-medium">{colTasks.length}</span>
                      </div>
                      <button
                        onClick={() => { setNewTaskColumn(col.id); setShowNewTask(true) }}
                    ...
                    <SortableContext items={colTasks.map((t) => t._id)} strategy={verticalListSortingStrategy}>
                      <DroppableColumn id={col.id}>
                        {colTasks.map((task) => (
                          <SortableTaskCard key={task._id} task={task} onClick={() => setSelectedTask(task)} />
                        ))}
